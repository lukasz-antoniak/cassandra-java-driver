...
public class CqlRequestHandler implements Throttled {
  ...
  private void sendRequest(
    ...
    if (channel == null) {
      ...
      trackNodeStart(statement, executionProfile, node, logPrefix);
  ...
  private void setFinalResult(
    ...
    try {
      ...
      if (result.complete(resultSet)) {
        ...
        long endTimeNanos = trackNodeEnd(callback, null, NANOTIME_NOT_MEASURED_YET, logPrefix);
        trackEnd(callback.node, null, endTimeNanos);
  ...
  private void setFinalError(Statement<?> statement, Throwable error, Node node, int execution) {
    ...
    if (result.completeExceptionally(error)) {
      cancelScheduledTasks();
      trackEnd(node, error, startTimeNanos);
  ...
  private class NodeResponseCallback
    ...
    public void operationComplete(Future<java.lang.Void> future) throws Exception {
      if (!future.isSuccess()) {
        ...
        if (error instanceof EncoderException
          ...
          trackNodeEnd(this, error.getCause(), NANOTIME_NOT_MEASURED_YET, logPrefix);
          ...
          trackNodeEnd(this, error, NANOTIME_NOT_MEASURED_YET, logPrefix);
          ((DefaultNode) node)
              .getMetricUpdater()
    ...
    public void onResponse(Frame responseFrame) {
      ...
      try {
        ...
        if (responseMessage instanceof SchemaChange) {
          ...
          trackNodeEnd(this, error, nodeResponseTimeNanos, logPrefix);
          setFinalError(statement, error, node, execution);
        }
      } catch (Throwable t) {
        trackNodeEnd(this, t, nodeResponseTimeNanos, logPrefix);
    ...
    private void processErrorResponse(Error errorMessage) {
      if (errorMessage.code == ProtocolConstants.ErrorCode.UNPREPARED) {
        ...
        trackNodeStart(reprepareRequest, executionProfile, node, logPrefix);
        reprepareHandler
            .start()
            .handle(
                (repreparedId, exception) -> {
                  if (exception != null) {
                    ...
                    if (exception instanceof UnexpectedResponseException) {
                        ...
                        if (prepareError instanceof QueryValidationException
                          ...
                          long endTimeNanos =
                              trackReprepareStatementEnd(
                                  ...
                                  reprepareStartNanos,
                                  logPrefix);
                          trackNodeEnd(this, prepareError, endTimeNanos, logPrefix);
                      ...
                      trackNodeEnd(this, exception, endTimeNanos, logPrefix);
                    ...
                    if (!repreparedId.equals(idToReprepare)) {
                      ...
                      trackNodeEnd(
                          this, illegalStateException, NANOTIME_NOT_MEASURED_YET, logPrefix);
  ...
  private void trackStart() {
    // TODO: Move execution profile calculation to constructor.
    DriverExecutionProfile executionProfile =
        Conversions.resolveExecutionProfile(initialStatement, context);
    trackStart(initialStatement, executionProfile, logPrefix);
  }

  /** Notify request tracker that processing of given statement starts. */
  private void trackStart(
      Request request, DriverExecutionProfile executionProfile, String logPrefix) {
  ...
  private void trackNodeStart(
      Request request, DriverExecutionProfile executionProfile, Node node, String logPrefix) {
    if (requestTracker instanceof NoopRequestTracker) {
      return;
    }
  ...
  private long trackNodeEnd(
    ...
    if (error == null) {
      requestTracker.onNodeSuccess(request, latencyNanos, executionProfile, node, logPrefix);
    } else {
      requestTracker.onNodeError(request, error, latencyNanos, executionProfile, node, logPrefix);
    }
    return endTimeNanos;
  }
  ...
  private long trackNodeEnd(
      NodeResponseCallback callback, Throwable error, long endTimeNanos, String logPrefix) {
    return trackNodeEnd(
        callback.statement,
        callback.executionProfile,
        callback.node,
   ...
   * statement.
   */
  private long trackReprepareStatementEnd(
      Request statement,
      NodeResponseCallback callback,
      Throwable error,
      long startTimeNanos,
    ...
    long endTimeNanos =
        trackNodeEnd(
            ...
            logPrefix);
    trackEnd(
        statement, callback.executionProfile, callback.node, error, startTimeNanos, endTimeNanos);
    return endTimeNanos;
  }
   ...
   * with error).
   */
  private void trackEnd(Node node, Throwable error, long endTimeNanos) {
    // TODO: Move execution profile calculation to constructor.
    DriverExecutionProfile executionProfile =
        Conversions.resolveExecutionProfile(initialStatement, context);
    trackEnd(initialStatement, executionProfile, node, error, startTimeNanos, endTimeNanos);
  }
  ...
  private void trackEnd(
    ...
    long latencyNanos = endTimeNanos - startTimeNanos;
    if (error == null) {
      requestTracker.onSuccess(request, latencyNanos, executionProfile, node, logPrefix);
    } else {
      requestTracker.onError(request, error, latencyNanos, executionProfile, node, logPrefix);
    }
  }
}
